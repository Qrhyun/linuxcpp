...
{
    ui->setupUi(this);
    ui->groupBox->setEnabled(false);//禁用数据输入输出组框
}
...
void MainWindow::on_btnFile_clicked()
{
    ...
    if(!m_filename.isEmpty())
    {
        ...
        ui->groupBox->setEnabled(true);//启用数据输入输出组框
    }
}
//选择font按钮,字体对话框
...
{
    QPalette plet=ui->btnColor_In->palette();//获取按钮的调色板
    QColor color=plet.buttonText().color();//调色板的按钮文本颜色
    color=QColorDialog::getColor(color);//选择颜色对话框
    if(color.isValid())
    {
        plet.setColor(QPalette::ButtonText,color);//设置调色板的按钮文本颜色
...
void MainWindow::on_btnInt_Read_clicked()
{
    qfloat16 value=0;
    readByStream(value);
    float val=value;    //转为float类型，QString的函数不支持qfloat16
